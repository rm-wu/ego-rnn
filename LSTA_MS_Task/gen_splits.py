...
def gen_split_(root_dir, test_id, stackSize=5, fmt='png'):
    trainDatasetF = []
    testDatasetF = []
    trainLabels = []
    testLabels = []
    ...
    for dir_id in range(4):
        dirF = os.path.join(root_dir, f'S{dir_id+1}')
        class_id = 0
        for target in sorted(os.listdir(dirF)):
            ...
            for inst in sorted(os.listdir(dirF1)):
                if not inst.startswith('.'):
                    ...
                    numFrames = len(glob.glob1(os.path.join(inst_dirF, 'rgb'), f'*.{fmt}'))
                    if numFrames >= stackSize:
                        if dir_id + 1 == test_id:
                            testDatasetF.append(os.path.join(inst_dirF, 'rgb'))
                            testLabels.append(class_id)
                            testNumFrames.append(numFrames)
                        else:
                            trainDatasetF.append(os.path.join(inst_dirF, 'rgb'))
...
def gen_split_(root_dir, test_id, stackSize=5):
    trainDatasetF = []
    testDatasetF = []
    trainLabels = []
    testLabels = []
